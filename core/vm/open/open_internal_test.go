...
var _ = Describe("Open", func() {
	...
	ioFlush := func(io task.IO, channels ...task.Channel) chan struct{} {
		ioDidFlush := make(chan struct{})
		go func() {
			defer close(ioDidFlush)
			io.Flush(nil, channels...)
		}()
		return ioDidFlush
	}
		...
		for _, entryCap := range tableCap {
			...
			Context("when trying to open", func() {
				Context("when there are insufficient messages", func() {
					It("should not write to the output", func() {
